...
class MultiviewDataset(Dataset):
    ...
    def load_normal_image(
        ...
        if camera_space:
            ...
            # Transform world space normal to camera space
            image = (
    ...
    def __getitem__(self, index):
        ...
        while retries < max_retries:
            try:
                ...
                if self.cfg.image_modality == "normal":
                    images = [
                        ...
                        for p, c in zip(image_paths, c2w)
                    ]
                else:
                    images = [
                        self.load_image(
                ...
                for modality in source_image_modality:
                    if modality == "position":
                        depth_paths = [os.path.join(scene_dir, f"depth_{f}.exr") for f in self.cfg.image_names]
                        # 检查 depth 文件
                        for p in depth_paths:
                            if not os.path.exists(p): raise FileNotFoundError(f"Depth not found: {p}")
                            
                        depth_masks = [
                            self.load_depth(p, self.cfg.height, self.cfg.width)
                            for p in depth_paths
                        ]
                        ...
                        if self.cfg.projection_type == "PERSP":
                            position_maps = get_position_map_from_depth(
                                depths, masks, intrinsics, c2w_, image_wh=(self.cfg.width, self.cfg.height),
                            )
                        elif self.cfg.projection_type == "ORTHO":
                            position_maps = get_position_map_from_depth_ortho(
                                depths, masks, c2w_, ortho_scale, image_wh=(self.cfg.width, self.cfg.height),
                            )
                        position_maps = ((position_maps + self.cfg.position_offset) / self.cfg.position_scale).clamp(0.0, 1.0)
                        source_images.append(position_maps)
                        
                    elif modality == "normal":
                        # 检查 normal 文件 (作为 condition 时)
                        norm_paths = [os.path.join(scene_dir, f"{modality}_{f}.{self.cfg.image_suffix}") for f in self.cfg.image_names]
                        for p in norm_paths:
                            if not os.path.exists(p): raise FileNotFoundError(f"Source normal not found: {p}")

                        normal_maps = [
                            self.load_normal_image(
                                p, height=self.cfg.height, width=self.cfg.width,
                                background_color=background_color,
                                camera_space=self.cfg.use_camera_space_normal, c2w=c,
                            )
                            for p, c in zip(norm_paths, c2w)
                        ]
                        source_images.append(torch.stack(normal_maps, dim=0))
                        
                    elif modality == "plucker":
                        if self.cfg.projection_type == "ORTHO":
                            plucker_embed = get_plucker_embeds_from_cameras_ortho(c2w, [ortho_scale] * len(c2w), self.cfg.width)
                        elif self.cfg.projection_type == "PERSP":
                            plucker_embed = get_plucker_embeds_from_cameras(c2w, [camera_angle_x] * len(c2w), self.cfg.width)
                        else:
                            raise NotImplementedError
                        plucker_embed = plucker_embed.permute(0, 2, 3, 1)
                        plucker_embed = ((plucker_embed + self.cfg.plucker_offset) / self.cfg.plucker_scale).clamp(0.0, 1.0)
                ...
                if self.reference_scenes is not None:
                    # 这里的 reference_scenes 可能也会有缺失，也需要 try-catch 保护
                    ...
                    # 随机选一张 ref
                    # 增加鲁棒性：遍历列表，直到找到一张存在的
                    valid_ref_path = None
                    random.shuffle(reference_image_paths) # 打乱顺序模拟随机选择
                    for rp in reference_image_paths:
                        if os.path.exists(rp):
                            valid_ref_path = rp
                            break
                    
                    if valid_ref_path is None:
                         raise FileNotFoundError(f"No valid reference image found in {reference_scene_dir}")
                    ...
                    if self.cfg.reference_augment_resolutions is None:
                        ...
                        random_resolution = random.choice(self.cfg.reference_augment_resolutions)
                        reference_image_ = Image.open(valid_ref_path).resize((random_resolution, random_resolution))
                        reference_image = self.load_image(
                            reference_image_,
                            height=self.cfg.height,
                            width=self.cfg.width,
                ...
                # [核心修复] 捕获所有加载异常
                print(f"[DataLoader Warning] Failed to load index {index} (Scene: {self.all_scenes[index]}). Error: {e}")
                print("Skipping and trying another random sample...")
                
                # 随机换一个样本重试
                index = random.randint(0, len(self.all_scenes) - 1)
                retries += 1
        
        # 如果重试10次都失败，为了不卡死程序，抛出异常
        raise RuntimeError(f"Failed to load valid data after {max_retries} retries. Check your dataset path.")
